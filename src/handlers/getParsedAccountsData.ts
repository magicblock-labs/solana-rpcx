  ...
  try {
    ...
    for (let i = 0; i < accountsResponse.result.value.length; i++) {
      ...
      if (account) {
        ...
        const idl = idls[i];
        if (!idl) {
          return errorResponse(body.id, -32602, "IDL not found for program", { programId: new PublicKey(account.owner).toString() });
        }
        const program = new Program(idl as Idl, provider);
        try {
          account.data = decodeAccount(dataBuffer, program);
        } catch {
          return errorResponse(body.id, -32602, "Account type not found for discriminator", { accountIndex: i, pubkey: pubkeys[i] });
        }
      }
    }
  } catch (error: unknown) {
